...
local function register_platform(on, preset)
	...
	if not ndef then
		minetest.log("warning",
			" register_platform couldn't find preset node "
			.. preset
		)
		return
	end
	...
	minetest.register_node(on .. ":platform_low_" .. nodename, {
		...
		tiles = {
			btex .. "^advtrains_platform.png",
			btex, btex, btex, btex, btex
		},
...
for _, entry in pairs(nodelist) do
	...
	for _, lengths in pairs(woodpath_lengths) do
		...
		local h = string.format(":" .. entry.name:gsub(" ", "_")
			.. "path_track_%02d%02d", b, f)
		local d = string.format(entry.name
			.. " level crossing %02d-%02d", b, f)
		local h2 = string.format(":" .. entry.name:gsub(" ", "_")
			.. "path_track_narrow_%02d%02d", b, f)
		local d2 = string.format(entry.name
			.. " level crossing (narrow) %02d-%02d", b, f)
